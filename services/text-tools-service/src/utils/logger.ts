...
const logsDir = path.join(__dirname, '../../../logs/text-tools-service');
...
const getLogFilePath = (): string => {
  return path.join(logsDir, `text-tools-${getCurrentDate()}.log`);
};
...
const cleanupOldLogs = (): void => {
  try {
    ...
    files.forEach(file => {
      if (file.startsWith('text-tools-') && file.endsWith('.log')) {
